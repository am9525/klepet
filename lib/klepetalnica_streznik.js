  ...
  socket.on('dregljaj', function (dregljaj) {
    ...
    if (dregljaj) {
      ...
      if (socketIdNaslovnika && socketIdNaslovnika != socket.id) {
        //poslji vzdevek
       io.sockets.sockets[socketIdNaslovnika].emit('dregljaj', { vzdevek: vzdevkiGledeNaSocket[socket.id] });
       io.sockets.sockets[socket.id].emit("sporocilo", { besedilo: "Dregljaj za "+dregljaj});
      } else {
       io.sockets.sockets[socket.id].emit('sporocilo', {
